...
from web_scraper.ken_pom import KenPomUtils
...
HEADERS = ("DATE", "FAVORITE", "UNDERDOG", "ACTUAL SPREAD", "PREDICTED SPREAD", "PERCENTAGE", "CORRECT TEAM",
           "COVERED")
...
class KenPomPage(BasePage):
    def __init__(self, driver):
        ...
        self.driver = driver
        self.f = open(os.getcwd() + "/../data/ken_pom/KenPomPredictions.txt", "w")
    ...
    def get_prediction(self, row_number) -> tuple:
        ...
        if team == teams[0].text:
            return self.get_current_date(), teams[0].text, teams[1].text, actual_scores[3] - actual_scores[1], \
                   score[1] - score[0], percentage, actual_scores[3] - actual_scores[1] < 0, teams[0]
        else:
            return self.get_current_date(), teams[1].text, teams[0].text, actual_scores[1] - actual_scores[3], \
                   score[1] - score[0], percentage, actual_scores[1] - actual_scores[3] < 0
    ...
    def write_game_data_to_file(self, end_date):
        while end_date not in self.driver.current_url:
            for rows in range(1, self.get_num_rows() + 1):
                ...
                s = KenPomUtils.ken_pom_tuple_to_json(row)
                self.f.write(str(s) + '\n')
